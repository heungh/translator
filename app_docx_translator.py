...
BASE_CONTEXT = (
    ...
    "- NEVER summarize, condense, or abridge the text. The output must contain the SAME number "
    "of paragraphs and sentences as the input.\n"
    "- PRESERVE ALL lines containing special characters such as <, >, [, ], 《, 》, 「, 」, 『, 』, "
    "【, 】, ―, ★, ☆, ♡, ♥, etc. These lines MUST appear in the output exactly as positioned in "
    "the input. Do NOT delete, skip, or merge any line that contains special characters.\n"
...
def _safe_text_for_api(text: str) -> str:
    """Wrap text with special characters to prevent them from being
    interpreted as markup/tags during API processing. Uses CDATA-like
    fencing so the model sees the raw characters."""
    # If text contains angle brackets that could be misinterpreted,
    # wrap in a code fence so the model treats them as literal text
...
def translate_with_claude(text: str, model_id: str, region: str, system_prompt: str = "") -> str:
    ...
    safe_text = _safe_text_for_api(text)
    message = {
    ...
    # Strip code fence if model echoed it back
...
# =============================================================================
# QA Validation — detect summarization & special-character line loss
# =============================================================================

# Regex pattern for lines that contain "meaningful" special characters authors use
...
def _extract_special_lines(text: str) -> list[str]:
    """Return list of lines (stripped) that contain special characters."""
    return [line.strip() for line in text.splitlines()
            if line.strip() and _SPECIAL_LINE_RE.search(line)]


class TranslationQAResult:
    """Holds the result of a QA check on a single chunk translation."""
    def __init__(self):
        self.passed = True
        self.issues: list[str] = []
        self.line_ratio: float = 1.0
        self.missing_special_lines: list[str] = []

    def __bool__(self):
        return self.passed


def validate_translation(source: str, translated: str,
                         line_ratio_threshold: float = 0.8) -> TranslationQAResult:
    """
    Validate translated text against source for:
      1. Content summarization (line count ratio drops below threshold)
      2. Missing special-character lines
    """
    result = TranslationQAResult()

    # --- Check 1: Line count ratio (anti-summarization) ---
    src_lines = _count_nonempty_lines(source)
    tgt_lines = _count_nonempty_lines(translated)

    if src_lines > 0:
        result.line_ratio = tgt_lines / src_lines
        if result.line_ratio < line_ratio_threshold:
            result.passed = False
            result.issues.append(
                f"Possible summarization: source has {src_lines} lines but "
                f"translation has {tgt_lines} lines (ratio={result.line_ratio:.2f})"
            )

    # --- Check 2: Special-character line preservation ---
    src_special = _extract_special_lines(source)
    if src_special:
        # For each special line in source, check if it (or a recognizable
        # portion) appears in the translated output
        for sline in src_special:
            # Extract just the special-char "skeleton" of the line
            specials_in_line = _SPECIAL_LINE_RE.findall(sline)
            # Check if any translated line contains these same special chars
            found = False
            for tline in translated.splitlines():
                if all(ch in tline for ch in specials_in_line):
                    found = True
                    break
            if not found:
                result.missing_special_lines.append(sline)

        if result.missing_special_lines:
            result.passed = False
            result.issues.append(
                f"Missing {len(result.missing_special_lines)} special-character line(s) "
                f"in translation"
            )
...
def build_retry_prompt(source: str, qa_result: TranslationQAResult) -> str:
    """Build an enhanced user prompt for re-translation after QA failure."""
    parts = [
        "CRITICAL RETRY INSTRUCTION: Your previous translation was rejected by quality control.\n"
    ]
    if qa_result.line_ratio < 0.8:
        parts.append(
            f"ISSUE: You summarized the content. The source has "
            f"{_count_nonempty_lines(source)} non-empty lines but your translation "
            f"only had {int(qa_result.line_ratio * _count_nonempty_lines(source))} lines. "
            f"You MUST translate EVERY line without omission.\n"
        )
    if qa_result.missing_special_lines:
        parts.append(
            "ISSUE: You deleted lines containing special characters. "
            "The following lines MUST appear in your translation:\n"
        )
        for ml in qa_result.missing_special_lines[:10]:  # cap at 10 examples
            parts.append(f"  → {ml}")
        parts.append("")

    parts.append("Now translate the following text completely, line by line:\n")
    parts.append(source)
    return "\n".join(parts)
...
def translate_document(paragraphs, model_name, region, ollama_url, chunk_size,
    ...
    qa_reports: list[dict] = []   # all chunks, pass or fail
    ...
    for ci, chunk in enumerate(chunks):
        ...
        retry_count = 0
        while not qa.passed and retry_count < max_retries:
            retry_count += 1
            logger.warning(
                "QA failed for chunk %d (attempt %d/%d): %s",
                ci + 1, retry_count, max_retries, "; ".join(qa.issues),
            )
            if progress_callback:
                progress_callback(
                    ci + 1, total,
                    f"Chunk {ci + 1}/{total} — QA retry {retry_count}/{max_retries}...",
                )
        ...
        qa_reports.append({
            ...
            "src_special_count": len(src_special),
            "missing_special_count": len(qa.missing_special_lines),
            "missing_special_lines": qa.missing_special_lines,
            "retries": retry_count,
            "issues": qa.issues,
        })
        ...
        if not qa.passed:
            logger.error(
                "QA still failing after %d retries for chunk %d: %s",
                max_retries, ci + 1, "; ".join(qa.issues),
            )
            qa_warnings.append({
                "chunk": ci + 1,
                "issues": qa.issues,
    ...
    return translated, qa_warnings, qa_reports
...
def main():
    ...
    if st.button("Translate", type="primary", use_container_width=True):
        ...
        try:
            ...
            # --- QA Validation Report (always shown) ---
            st.divider()
            total_chunks = len(qa_reports)
            ...
            if failed_chunks == 0:
                ...
                st.error(f"QA Validation: {failed_chunks}/{total_chunks} chunk(s) FAILED")
                ...
                for r in qa_reports:
                    ...
                    if r["issues"]:
                        for issue in r["issues"]:
                            st.markdown(f"  - {issue}")
                    if r["missing_special_lines"]:
                        st.markdown("  **Missing special-character lines:**")
                        for ml in r["missing_special_lines"]:
                            st.code(ml, language=None)
